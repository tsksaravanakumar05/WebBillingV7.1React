...
const PattyCommissionReportUrl = "/api/PurchaseReportApp/PurchaseBillConsolidateReport";
...
export default function PattyCommissionReport() {
  ...
  useEffect(() => {
    ...
    setSession({
      ...
      GroupCommission: toBool(main0.GroupCommission),
    });
  ...
  const handleView = useCallback(async () => {
    if (!selectedSupplier?.value) {
      setMsg({ text: "Select The Supplier", isErr: true });
      return;
    }
    ...
    try {
      ...
      if (res?.ok === true || res?.IsSuccess === true) {
        ...
        openReportViewer(
          {
            ReportName: "PattyCommission",
            CacheKey: cacheKey,
            Fromdate,
            Todate,
            GroupBy: selectedSupplier.value,
            ReportType: "BOTH",
            Patty: session.CMBTPatty ? 1 : 0,
      ...
      <>
        ...
        <div className="so-layout">
          <div className="so-card">
            <div className="so-card-body">Loading...</div>
          </div>
        </div>
      </>
    );
        ...
        <div className="so-layout">
          <div className="so-card">
            ...
            <div className="so-card-body">
              <div className="so-report-title">Patty Commission - Report</div>
              ...
              <div className="so-content">
                <div className="so-left">
                  <label className="so-radio-row">
                    <input type="radio" name="patty-report-type" checked readOnly />
                    Both
                  </label>

                  <div className="so-basis-row">
                    <label className="so-checkbox">
                      <input type="checkbox" checked={session.CMBTPatty} readOnly />
                    ...
                    <label className="so-checkbox">
                      <input type="checkbox" checked={session.GroupCommission} readOnly />
                      <span className="so-checkbox-box">
                        <svg viewBox="0 0 24 24" fill="none" stroke="var(--clr-text-white)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                          <polyline points="20 6 9 17 4 12" />
                        </svg>
                ...
                <div className="so-right">
                  ...
                  <div className="so-field">
                    <label className="so-label" htmlFor="pc-fromdate">From Date</label>
                    <DateFieldDDMMYYYY id="pc-fromdate" value={fromDate} onChange={setFromDate} disabled={loading} />
                  </div>

                  <div className="so-field">
                    <label className="so-label" htmlFor="pc-todate">To Date</label>
                    <DateFieldDDMMYYYY id="pc-todate" value={toDate} onChange={setToDate} disabled={loading} />
                  </div>
                </div>
              </div>
