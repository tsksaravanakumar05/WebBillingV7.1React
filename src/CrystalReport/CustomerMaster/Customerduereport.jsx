...
export default function CustomerDueReport() {
  ...
  // ── Linked API-backed select — same searchable-popup pattern used in
  ...
  // (including the existing blank-value filter below), and the
  // onChange({ value, label }) contract are unchanged — only the selection
  // UI changed. `id` is accepted so the two linked pickers (Customer Name /
  // Mobile No) keep distinct label/button ids.
  const ApiSelect = ({ id, url, payload, headers = {}, labelKey, valueKey, value, onChange, placeholder }) => {
    ...
    return (
      <div className="so-field">
        <label className="so-label" htmlFor={id}>{placeholder.replace("Select ", "")}</label>
        <div className="so-select-wrap" ref={wrapRef}>
          ...
          >
            <span className={`so-select-btn-text${!value ? " is-placeholder" : ""}`}>
              {listLoading ? "Loading..." : value?.label || placeholder}
            </span>
            {value && !listLoading && (
              <span
  ...
  const styles = `
    ...
    /* Searchable select (Customer Name / Mobile No) */
