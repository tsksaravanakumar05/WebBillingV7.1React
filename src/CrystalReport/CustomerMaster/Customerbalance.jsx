...
export default function CustomerBalance() {
  ...
  // UI changed. `id` is still accepted/used so the four combos rendered on
  // this page (Customer / Area / Mobile No / Salesman) keep distinct ids.
  const ApiSelect = ({ id, url, payload, headers = {}, labelKey, valueKey, value, onChange, placeholder }) => {
    ...
    const popup =
      open && !loadingList && popRect ? (
        <div
          ref={popRef}
          className="cb-select-pop cb-select-pop-portal"
          style={{ top: popRect.top, left: popRect.left, width: popRect.width }}
        >
          <div className="cb-select-search-wrap">
            <input
              ref={searchRef}
              type="text"
              className="cb-select-search"
          ...
          <div className="cb-select-list">
            {filtered.length === 0 ? (
              <div className="cb-select-empty">No matches found</div>
            ) : (
              filtered.map((o) => (
                <div
                  key={o[valueKey]}
                  className={`cb-select-opt${
    ...
    return (
      <div className="cb-field">
        ...
        <div className="cb-select-wrap" ref={wrapRef}>
          <button
            type="button"
            id={id}
            ref={btnRef}
            className="cb-input cb-select-btn"
            disabled={loadingList}
            onClick={handleToggle}
          >
            <span className={`cb-select-btn-text${!value ? " is-placeholder" : ""}`}>
              {loadingList ? "Loading..." : value?.label || placeholder}
            </span>
            {value && !loadingList && (
              <span
                className="cb-select-clear"
                onClick={handleClear}
                role="button"
            ...
            <span className="cb-select-caret" aria-hidden="true">▾</span>
          </button>
  ...
  const styles = `
    ...
    .cb-input:disabled { background: #f5f6f8; cursor: not-allowed; }

    /* Searchable select (Customer / Area / Mobile No / Salesman) */
    .cb-select-wrap { position: relative; width: 100%; }
    .cb-select-btn { display: flex; align-items: center; gap: 8px; cursor: pointer; text-align: left; font-family: inherit; }
    .cb-select-btn:disabled { cursor: not-allowed; }
    .cb-select-btn-text { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .cb-select-btn-text.is-placeholder { color: #8a94a3; }
    .cb-select-clear { flex-shrink: 0; width: 16px; height: 16px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 10px; color: #8a94a3; transition: background .15s, color .15s; }
    .cb-select-clear:hover { background: #fff0f0; color: #dc3545; }
    .cb-select-caret { flex-shrink: 0; font-size: 10px; color: #8a94a3; }

    /* Base popup look. Positioning differs for the portal variant below. */
    .cb-select-pop { position: absolute; top: calc(100% + 6px); left: 0; right: 0; z-index: 40; background: #fff; border: 1px solid #c7cdd6; border-radius: 8px; box-shadow: 0 8px 24px rgba(26,43,80,.16); overflow: hidden; }
    ...
    .cb-select-pop-portal { position: fixed; right: auto; z-index: 3000; max-height: min(320px, calc(100vh - 24px)); display: flex; flex-direction: column; }

    .cb-select-search-wrap { padding: 8px; border-bottom: 1px solid #e8ecf0; flex-shrink: 0; }
    .cb-select-search { width: 100%; height: 32px; border: 1px solid #c7cdd6; border-radius: 4px; padding: 0 10px; font-size: 13px; color: #1e2d3d; box-sizing: border-box; outline: none; transition: border-color .15s, box-shadow .15s; }
    .cb-select-search:focus { border-color: #1a56db; box-shadow: 0 0 0 3px rgba(26,86,219,.15); }
    .cb-select-list { max-height: 260px; overflow-y: auto; }
    .cb-select-pop-portal .cb-select-list { max-height: none; overflow-y: auto; flex: 1; }
    .cb-select-opt { padding: 8px 12px; font-size: 13px; color: #1e2d3d; cursor: pointer; transition: background .12s; }
    .cb-select-opt:hover { background: #eef3ff; }
    .cb-select-opt.is-selected { background: #e3ecff; color: #1a4fd1; font-weight: 600; }
    .cb-select-empty { padding: 14px 12px; font-size: 13px; color: #8a94a3; text-align: center; }
        ...
        <div className="cb-layout">
          <div className="cb-panel">
            <div className="cb-card-header">
              ...
              <button type="button" className="cb-close-x" aria-label="Close" onClick={() => navigate("/dashboard")}>✕</button>
            </div>
