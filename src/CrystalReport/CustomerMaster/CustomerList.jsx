...
export default function CRMCustomer() {
  ...
  // ── Combo box — same ApiSelect pattern used across converted pages ──────
  ...
  const ApiSelect = ({ url, payload, headers = {}, labelKey, valueKey, value, onChange, placeholder }) => {
    ...
    const popup =
      open && !loadingList && popRect ? (
        <div
          ref={popRef}
          className="cc-select-pop cc-select-pop-portal"
        ...
        >
          ...
          <div className="cc-select-list">
            {filtered.length === 0 ? (
              <div className="cc-select-empty">No matches found</div>
            ) : (
              filtered.map((o) => (
                <div
                  key={o[valueKey]}
    ...
    return (
      <div className="cc-field">
        ...
        <div className="cc-select-wrap" ref={wrapRef}>
          <button
            type="button"
            id="cc-customer"
            ref={btnRef}
            className="cc-input cc-select-btn"
            disabled={loadingList}
            onClick={handleToggle}
          >
            <span className={`cc-select-btn-text${!value ? " is-placeholder" : ""}`}>
            ...
            <span className="cc-select-caret" aria-hidden="true">▾</span>
          </button>
  ...
  const styles = `
    ...
    .cc-input:disabled { background: #f5f6f8; cursor: not-allowed; }

    /* Searchable select (Customer / Supplier / Brand / Category / Group, etc.) */
    .cc-select-wrap { position: relative; width: 100%; }
    .cc-select-btn { display: flex; align-items: center; gap: 8px; cursor: pointer; text-align: left; font-family: inherit; }
    .cc-select-btn:disabled { cursor: not-allowed; }
    .cc-select-btn-text { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .cc-select-btn-text.is-placeholder { color: #8a94a3; }
    .cc-select-clear { flex-shrink: 0; width: 16px; height: 16px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 10px; color: #8a94a3; transition: background .15s, color .15s; }
    .cc-select-clear:hover { background: #fff0f0; color: #dc3545; }
    .cc-select-caret { flex-shrink: 0; font-size: 10px; color: #8a94a3; }

    /* Base popup look. Positioning differs for the portal variant below. */
    .cc-select-pop { position: absolute; top: calc(100% + 6px); left: 0; right: 0; z-index: 40; background: #fff; border: 1px solid #c7cdd6; border-radius: 8px; box-shadow: 0 8px 24px rgba(26,43,80,.16); overflow: hidden; }
    ...
    .cc-select-pop-portal { position: fixed; right: auto; z-index: 3000; max-height: min(320px, calc(100vh - 24px)); display: flex; flex-direction: column; }

    .cc-select-search-wrap { padding: 8px; border-bottom: 1px solid #e8ecf0; flex-shrink: 0; }
    .cc-select-search { width: 100%; height: 32px; border: 1px solid #c7cdd6; border-radius: 4px; padding: 0 10px; font-size: 13px; color: #1e2d3d; box-sizing: border-box; outline: none; transition: border-color .15s, box-shadow .15s; }
    .cc-select-search:focus { border-color: #1a56db; box-shadow: 0 0 0 3px rgba(26,86,219,.15); }
    .cc-select-list { max-height: 260px; overflow-y: auto; }
    .cc-select-pop-portal .cc-select-list { max-height: none; overflow-y: auto; flex: 1; }
    .cc-select-opt { padding: 8px 12px; font-size: 13px; color: #1e2d3d; cursor: pointer; transition: background .12s; }
    .cc-select-opt:hover { background: #eef3ff; }
    .cc-select-opt.is-selected { background: #e3ecff; color: #1a4fd1; font-weight: 600; }
    .cc-select-empty { padding: 14px 12px; font-size: 13px; color: #8a94a3; text-align: center; }
        ...
        <div className="cc-layout">
          <div className="cc-panel">
            <div className="cc-card-header">
              ...
              <button type="button" className="cc-close-x" aria-label="Close" onClick={() => navigate("/dashboard")}>✕</button>
            </div>
