...
const NAV_ITEMS = [
  ...
  { value: REPORT_TYPES.HOURLY_PROFIT,         label: "Hourly Profit Report",          icon: "⌛" },
...
export default function SaleReportPart1() {
  ...
  // customerList / salesmanList / cashierList / counterList / saleTypeList
  // are now loaded centrally by CC.useReportCombos() below — see the
  // "Load combo sources" useEffect replacement further down.
  ...
  // Single source of truth now — see CC.useReportCombos() in Common.jsx.
  // It reuses the SAME CC.api() + authHeaders() as every other call in this
  // file, and redirects to Login automatically if the backend reports a
  // dual-login / invalid-session (406) instead of leaving dropdowns empty.
  const {
    customerList,
    salesmanList,
    cashierList,
...
// ═══ Sale Bill Details Report / Cancel Sale Bill Details ═══
  ...
  const res = await CC.api(EP.SaleBillDetailsReport, null, {}, {
  ...
  if (res.ok) {
    ...
    setMsg({ text: "No Record !!!.", isErr: true });
      ...
      // ═══ Item Wise Profit Report ═══
        ...
        const res = await CC.api(EP.ItemWiseProfitReport, null, {}, {
          Daily: daily, GroupBy: gb, GroupByText: gbt, Fromdate, Todate, Comid, MComid,
        });
        if (res.ok) {
          openReportViewer("ItemWiseProfitReport", { Daily: daily, GroupBy: gb, Fromdate, Todate, ReportType: ReportTypenew, ReportTitle, CName, CAddress, CPhone });
        } else {
          setMsg({ text: "No Record !!!.", isErr: true });
        }
        setGroupBy(""); setGroupByText("");
      ...
      // ═══ Day Wise Profit Report ═══
        ...
        const res = await CC.api(EP.DayWiseProfitReport, null, {}, { Fromdate, Todate, Comid, MComid });
        if (res.ok) {
          openReportViewer("DayWiseProfitReport", { Fromdate, Todate, ReportType: ReportTypenew, ReportTitle, CName, CAddress, CPhone });
        } else {
          setMsg({ text: "No Record !!!.", isErr: true });
        }
      }
      ...
      // ═══ Salesman Itemwise Report ═══
        ...
        const res = await CC.api(EP.SalesManItemwiseReport, null, {}, {
          GroupByText: gbt, Fromdate, Todate, Comid, MComid, ReportType: SubReportType,
        });
        if (res.ok) {
          openReportViewer("SalesManItemwiseReport", {
            Daily: daily, GroupBy, Fromdate, Todate, ReportType: ReportTypenew, ReportTitle,
      ...
      // ═══ Day Wise Sale Report ═══
        ...
        const ReportTitle = "Day Wise Profit";
        const res = await CC.api(EP.DayWiseSaleReport, null, {}, { Fromdate, Todate, Comid, ReportType: SubReportType });
        if (res.ok) {
          openReportViewer("DayWiseSaleReport", {
      ...
      // ═══ Hourly Sale / Hourly Itemwise Report ═══
        ...
        const res = await CC.api(EP.HourlyItemwiseAmountReport, null, {}, { Amount, Fromdate, Todate, Comid, MComid });
        if (res.ok) {
      ...
      // ═══ Hourly Profit Report ═══
        ...
        const res = await CC.api(EP.HourlyItemwiseSaleAmountReport, null, {}, { Fromdate, Todate, Comid });
        if (res.ok) {
          openReportViewer("HourlywiseProfitReport", {
            rdbHourlyUpto: hourlyUpto, Daily: daily, GroupBy, Fromdate, Todate, ReportType: ReportTypenew, ReportTitle, CName, CAddress, CPhone,
          });
        } else {
          setMsg({ text: "No Record !!!.", isErr: true });
        }
      ...
      // ═══ Cancel Bill Summary Report ═══
        ...
        const res = await CC.api(EP.CancelBillLogReport, null, {}, { Fromdate, Todate, Comid });
        if (res.ok) {
          openReportViewer("CancelBillLogSummaryReport", { Daily: daily, Fromdate, Todate, ReportType: ReportTypenew, ReportTitle, CName, CAddress, CPhone });
        } else {
          setMsg({ text: "No Record !!!.", isErr: true });
        }
      }
      ...
      // ═══ Modified Bill Report (Consolidate / Detail) ═══
        ...
        if (consolidateOrDetail === "CONS") {
          ...
          const res = await CC.api(EP.ModifiedBillReport, null, {}, { Fromdate, Todate, Comid });
          if (res.ok) {
            openReportViewer("ModifiedBillReport", { Daily: daily, GroupBy, Fromdate, Todate, ReportType: ReportTypenew, ReportTitle, CName, CAddress, CPhone });
          } else {
            setMsg({ text: "No Record !!!.", isErr: true });
          }
        } else {
          const ReportTitle = "Modified Bill Detail Report";
          const res = await CC.api(EP.ModifiedBillDetailsReport, null, {}, { Fromdate, Todate, Comid, MComid });
          if (res.ok) {
      ...
      // ═══ Billwise Profit Report ═══
        ...
        const res = await CC.api(EP.BillWiseProfitReport, null, {}, { GroupByText, Fromdate, Todate, Comid, MComid });
        if (res.ok) {
          openReportViewer("BillWiseProfitReport", { Daily: daily, GroupBy, Fromdate, Todate, ReportType: ReportTypenew, ReportTitle, TaxSuppressId, CName, CAddress, CPhone });
        } else {
          setMsg({ text: "No Record !!!.", isErr: true });
        }
      }
      ...
      // ═══ All Type Report (requires F11 Group selection) ═══
        ...
        const res = await CC.api(EP.AllTypeOfTotalReport, null, {}, { GroupBy: gb, Fromdate, Todate, Comid, MComid });
        if (res.ok) {
      ...
      // ═══ Customer Bill Summary Report ═══
        ...
        const res = await CC.api(EP.CustomerBillSummaryReport, null, {}, { GroupByText, Fromdate, Todate, Comid });
        if (res.ok) {
          openReportViewer("CustomerBillSummaryReport", { Daily: daily, GroupBy, Fromdate, Todate, ReportType: ReportTypenew, ReportTitle, CName, CAddress, CPhone });
        } else {
          setMsg({ text: "No Record !!!.", isErr: true });
        }
      }
  ...
  return (
    <>
      ...
      <div className="sr-shell">
        ...
        {/* ── ItemGroup picker dialog (F11) ── */}
        {showGroupPopup && (
          <div className="mp-picker-ov" onClick={() => setShowGroupPopup(false)}>
            <div className="mp-picker" style={{ width: 550, height: 450 }} onClick={(e) => e.stopPropagation()}>
              <header>
                <h3>ProductGroup</h3>
