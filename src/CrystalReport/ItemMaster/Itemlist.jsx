...
//  footprint (uses the .so-input class) but adds an instant-filter text popup
//  so users can type to narrow long master-data lists instead of scrolling.
//  Selection/clear behaviour matches the original <select onChange> exactly —
//  it just calls onChange(value) with "" for "-- Select --" like before.
// ─────────────────────────────────────────────────────────────────────────────
function SearchableSelect({ id, options, labelKey = "label", value, onChange, disabled, placeholder = "-- Select --" }) {
  ...
  // (including external resets like the Refresh button / group-field switch).
  useEffect(() => {
          ...
          <div className="so-card">
            ...
            {groupField && (
              <div className="so-form-grid">
                ...
                <SearchableSelect
                  id="il-combo"
                  options={activeCombo.list}
                  labelKey={activeCombo.labelKey}
                  value={selections[groupField]?.value ?? ""}
                  onChange={(val) => handleComboChange(groupField, val, activeCombo.list)}
                  placeholder="-- Select --"
                />
              </div>
            )}
