...
export default function SupplierDuePayment() {
  ...
  // positioned inside the card. Same external contract as before:
  // value={value}/onChange({value,label}) — only the picker UI changed, so
  // it keeps working without touching any surrounding logic.
  const ApiSelect = ({ url, payload, headers = {}, labelKey, valueKey, value, onChange, placeholder }) => {
    ...
    return (
      <div className="so-field">
        <label className="so-label" htmlFor="sd-supplier">{placeholder.replace("Select ", "")}</label>
  ...
  const styles = `
    ...
    .so-field { display: flex; align-items: center; gap: 14px; margin-bottom: 8px; }
        ...
        <div className="so-layout">
          <div className="so-card">
            <div className="so-card-header">
              <div className="so-card-header-title">Supplier Due Payment</div>
              <button type="button" className="so-close-x" aria-label="Close" onClick={() => navigate(-1)}>✕</button>
            </div>

            <div className="so-card-body">
              <div className="so-report-title">Supplier Due Payment - Report</div>
