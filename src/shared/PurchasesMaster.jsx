...
const loadColConfig = useCallback(async () => {
 ...
 const res = await fetch(
      CC.BASE_URL + `${CC1.GetFocusColumnsUrl}?comid=${sess.Comid}&filename=Purchase`,
      {
        method: "POST",
        headers: {
...
const handleFocusToggle = useCallback((idx, field, val) => {
  ...
  // ── Original values for StockDetails (Edit reverse logic) ──
...
const stockDetails = editId > 0
  ? gridRows
      .filter((r) => r.ProductCode !== "" && valNum(r._origItemQty) > 0)
      .map((r) => ({
        ProductRefid: parseInt(r.ProductRefId, 10) || 0,
        Batchid:      parseInt(r._origBatchRefId, 10) || 0,
        RealQty:      valNum(r._origItemQty),     // old qty
        Qty:          0.0,
        MfDate:       r._origMfgDate || "",
        ExpDate:      r._origExpiryDate || "",
...
/>
  ...
  if (e.key === "Enter") {
    e.preventDefault();
    // Find the last (or first empty) row in the grid
    const targetRow = gridRows[gridRows.length - 1];
    if (targetRow) {
      const el = document.getElementById(`cell_${targetRow._key}_ProductCode`);
      if (el) { el.focus(); el.select?.(); }
    }
  }
}}
