...
export default function SupplierPayment() {
  ...
  const doDeleteAfterPwd = useCallback(async (idx) => {
    ...
    if (!ok) { addRow(); return; }
    setLoading(true);
    const res = await CC.api(CC.DeleteSupplierPayment, null, {},
      { Id: Number(id), Comid: Number(sess.Comid), MirrorTable: Number(sess.MirrorTable), Updateid: String(updateId || "") }
    );
    setLoading(false);
    if (redirectIfDualLogin(res)) return;
    if (res.ok) {
  ...
  const paymentSave = useCallback(async (pendingDetails = []) => {
    ...
    setLoading(true);
    const res = await CC.insertapi(CC.InsertSupplierPayment, payload, {
  ...
  const openPendingBills = useCallback(async (rowIdx, sid) => {
    ...
    setLoading(false);
    if (redirectIfDualLogin(res)) return;
    const bills = Array.isArray(res?.data) ? res.data : Array.isArray(res) ? res : [];
