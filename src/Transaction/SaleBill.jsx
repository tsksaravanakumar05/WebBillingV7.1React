  ...
  <div className="popup-window f12-popup">
    ...
    <div className="popup-body">
      ...
      <thead><tr><th style={{ width: 180 }}>Column</th><th style={{ width: 70, textAlign: "center" }}>Visible</th><th style={{ width: 70, textAlign: "right" }}>Width</th></tr></thead>
        ...
        <tbody>
          {local.map((c, i) => (
            <tr key={c.key} className={i % 2 === 0 ? "mp-sale-f12-row-even" : "mp-sale-f12-row-odd"}>
    ...
    <div className="popup-overlay">
      <div className="popup-window product-popup">
        <div className="popup-header">
          ...
          <button className="popup-close" onClick={onClose} title="Close (Esc)">✕</button>
        </div>
        <div className="popup-body">
          <input
            ref={inputRef}
          ...
          <div className="popup-list-wrap">
            <table className="popup-table">
              {isTamil ? (
                <thead>
                  <tr><th>Code</th><th>Description</th><th>TamilName</th></tr>
                </thead>
              ) : (
                <thead>
                  <tr><th>Code</th><th>Description</th><th>UOM</th><th>MRP</th><th>SaleRate</th><th>GST%</th></tr>
                </thead>
              )}
              <tbody ref={listRef}>
                {filtered.map((p, idx) => (
                  <tr key={p.Id} data-idx={idx}
                    ...
                    {isTamil ? (
                      <>
                        <td>{p.Prod_Code ? p.Prod_Code : p.ProductCode}</td>
                        <td>{p.PName ? p.PName : p.ProductName}</td>
                        <td>{p.PrinterName || "—"}</td>
                      </>
                    ) : (
                      <>
                        ...
                        <td>{p.PName ? p.PName : p.ProductName}</td>
                        <td>{p.UOM || "—"}</td>
                        <td className="right">{CC.f2(CC.vn(p.MRP)).toFixed(2)}</td>
                        <td className="right">{CC.f2(CC.vn(p.SaleRate ? p.SaleRate : p.SalesRate)).toFixed(2)}</td>
                        <td className="right">{CC.f2(CC.vn(p.GST)).toFixed(2)}</td>
                      </>
                    )}
                  </tr>
                ))}
                ...
                {createVisible && (
                  <tr
                    data-idx={filtered.length}
                    className={hilite === filtered.length ? "popup-row selected" : "popup-row"}
                    onClick={() => onCreateProduct?.(q.trim())}
                    onMouseEnter={() => setHilite(filtered.length)}
                  >
                    <td colSpan={isTamil ? 3 : 6} style={{ fontWeight: 700, color: "var(--clr-primary, #1f65de)" }}>
                      + Create New Product: {q.trim()}
                    </td>
                  </tr>
                )}
                ...
                {filtered.length === 0 && !createVisible && (
                  <tr><td colSpan={isTamil ? 3 : 6} className="no-data">No products found</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
const loadFocusCols = useCallback(async (mcomid) => {
  ...
  function LotNoPopup({ lots, onSelect, onClose }) {
    const [search, setSearch] = useState("");
    const [hilite, setHilite] = useState(0);
    const listRef = useRef(null);
    const filteredLots = lots.filter(item =>
      String(item.LotNo || "").toUpperCase().includes(search.toUpperCase())
    );
    const handleLotKeyDown = useCallback((e) => {
      if (e.key === "ArrowDown") {
        e.preventDefault();
        setHilite(h => Math.min(h + 1, Math.max(filteredLots.length - 1, 0)));
      }
      if (e.key === "ArrowUp") {
        e.preventDefault();
        setHilite(h => Math.max(h - 1, 0));
      }
      if (e.key === "Enter" && filteredLots[hilite]) {
        e.preventDefault();
        onSelect(filteredLots[hilite]);
      }
      if (e.key === "Escape") onClose();
    }, [filteredLots, hilite, onClose, onSelect]);
    ...
    return (
      <div style={{
        position: "fixed", inset: 0, background: "rgba(10,20,40,.35)",
        display: "flex", alignItems: "center", justifyContent: "center", zIndex: 8850,
      }}>
        <div style={{
          background: "#fff", borderRadius: 10, width: 760, maxHeight: 460,
          display: "flex", flexDirection: "column", overflow: "hidden",
          boxShadow: "0 16px 48px rgba(31,101,222,.22)", border: "1px solid #d0ddf5",
        }}>
          <div style={{
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: "#fff", flex: 1 }}>Lot No Details</span>
            <span style={{ fontSize: 10, fontWeight: 600, color: "rgba(255,255,255,.65)", background: "rgba(255,255,255,.15)", borderRadius: 10, padding: "2px 8px" }}>
              {filteredLots.length} lots
            </span>
            <button onClick={onClose} style={{ background: "rgba(255,255,255,.15)", border: "none", color: "#fff", width: 22, height: 22, borderRadius: "50%", cursor: "pointer", fontSize: 11 }}>x</button>
          </div>
          <div style={{ padding: 10, borderBottom: "1px solid #e2e8f0", background: "#f8fbff" }}>
            <input
              className="sb-inp"
              value={search}
          ...
          <div style={{
            display: "grid", gridTemplateColumns: "110px 70px 80px 80px 80px 80px 80px 80px",
            padding: "4px 10px", background: "#f0f4fc", borderBottom: "1px solid #dde6f5",
            fontSize: 10, fontWeight: 700, color: "#6b7a99", textTransform: "uppercase", letterSpacing: ".4px",
          }}>
            <span>Lot No</span><span>LotMark</span><span style={{ textAlign: "right" }}>SaleKg</span><span style={{ textAlign: "right" }}>SaleBags</span>
            <span style={{ textAlign: "right" }}>PurBags</span><span style={{ textAlign: "right" }}>PurKg</span><span style={{ textAlign: "right" }}>Bal Bags</span><span style={{ textAlign: "right" }}>Bal Kg</span>
          </div>
          <div ref={listRef} style={{ overflowY: "auto", flex: 1 }}>
            {filteredLots.length === 0 ? (
              <div style={{ textAlign: "center", color: "#94a3b8", padding: 20, fontSize: 12 }}>No lot records found</div>
            ) : filteredLots.map((item, idx) => {
              const balBags = CC.vn(item.Bags) - CC.vn(item.SaleBags);
              const balKg = CC.vn(item.ItemQty) - CC.vn(item.SaleKg);
              return (
                <div
                  key={`${item.Id || idx}-${item.LotNo || ""}`}
                  data-idx={idx}
                  tabIndex={0}
                ...
                >
                  <span style={{ color: "#1f65de", fontWeight: 700 }}>{item.LotNo || "-"}</span>
                  <span style={{ color: "#475569" }}>{item.Mark || "-"}</span>
                  <span style={{ textAlign: "right" }}>{CC.vn(item.SaleKg).toFixed(2)}</span>
                  <span style={{ textAlign: "right" }}>{CC.vn(item.SaleBags).toFixed(0)}</span>
                  <span style={{ textAlign: "right" }}>{CC.vn(item.Bags).toFixed(0)}</span>
                  <span style={{ textAlign: "right" }}>{CC.vn(item.ItemQty).toFixed(2)}</span>
                  <span style={{ textAlign: "right", color: balBags <= 0 ? "#dc2626" : "#1a2e4a" }}>{balBags.toFixed(0)}</span>
                  <span style={{ textAlign: "right", color: balKg <= 0 ? "#dc2626" : "#1a2e4a" }}>{balKg.toFixed(2)}</span>
                </div>
              );
            })}
          </div>
...
const getRowEnabledCols = useCallback((rid) => {
  ...
  const applyLotNoToRow = useCallback((rid, lotItem) => {
    setRows(prev => prev.map(r => (
      r._rid !== rid
        ...
        : {
            ...r,
            PDRefid: lotItem.Id || r.PDRefid || null,
            Bat_No: lotItem.LotNo || r.Bat_No || "",
            _dirty: true,
          }
    )));
    setLotPopup(null);
  ...
  const openLotNoPopup = useCallback(async (rid, item) => {
    const lotRes = await CC.api(CC.SelectLotNoDetails, null, {}, { Id: item.Id, Supid: 0 });
    if (redirectIfDualLogin(lotRes)) return false;
    const lotList = Array.isArray(lotRes.data) ? lotRes.data
                  : Array.isArray(lotRes.Data1) ? lotRes.Data1 : [];
    if (lotList.length === 0) return false;
    fillItemIntoRow(rid, item, { skipAdvance: true });
    setLotPopup({ rid, lots: lotList });
    return true;
  ...
  const fetchProductByCode = useCallback(async (rid, code) => {
    if (!code.trim()) return;
    const usePattyLotPopup = sess.CMBTPatty ;
    const payload = {
      code: code.trim().toUpperCase(),
      Comid: sess.MComid, CComid: sess.Comid,
      Id: 0, Batchwise: usePattyLotPopup ? 0 : 1,
    };
    const res = await CC.api(CC.SO_SelectItemByCodeUrl, null, {}, payload);
    if (redirectIfDualLogin(res)) return;
    ...
    if (arr.length === 1) {
      if (usePattyLotPopup && CC.vn(arr[0]?.PattyItem) === 1) {
        const opened = await openLotNoPopup(rid, arr[0]);
        if (opened) return;
      }
...
>
  ...
  {loading && <span style={{ fontSize: 11, color: "#6b7a99" }}>⏳ {ldMsg}</span>}
      ...
      {lotPopup && (
        <LotNoPopup
          lots={lotPopup.lots}
          onSelect={item => { applyLotNoToRow(lotPopup.rid, item); }}
          onClose={() => setLotPopup(null)}
        />
      )}
