...
//   import "./PurchaseOrder.css";
  ...
  function calcPurchaseRow(row, igstBill) {
    ...
    if (qty === 0 || purrate === 0) {
      return { ...row, Amount: 0, TaxAmt: 0, CTAmount: 0, STAmount: 0,
               DiscountAmt: "0.00", cdAmount: "0.00", LandingCost: "0.00", ProductTotal: 0 };
    }
  ...
  function PwModal({ title, comid, onOk, onClose }) {
    ...
    const submit = async () => {
        ...
        try {
          ...
          if (data.ok ?? data.IsSuccess ?? false) {
            onOk();
            onClose();
          } else {
      ...
      <div className="mp-ov" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
        <div className="mp-modal-box" style={{ width: 220 }}>
          ...
          <div className="mp-modal-body" style={{ padding: "14px 16px" }}>
            <input ref={inpRef} type="password" value={pwd} onChange={e => { setPwd(e.target.value); setErr(""); }}
      ...
      <div className="mp-ov" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
        <div className="mp-modal-box sb-f5-modal" style={{ width: 1050, maxHeight: "85vh", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", background: "#f5f9ff", borderBottom: "1px solid #dde6f5", flexShrink: 0 }}>
            ...
            <select value={suppId} onChange={e => setSuppId(e.target.value)} style={{ height: 28, border: "1px solid #c5d8f8", borderRadius: 4, padding: "0 6px", fontSize: 12, minWidth: 140 }}>
              <option value="0">All Suppliers</option>
              {(suppliers || []).map(s => <option key={s.Id} value={s.Id}>{s.AccountName}</option>)}
            </select>
            <button className="mp-btn sv" style={{ height: 28, padding: "0 14px", fontSize: 11 }} onClick={() => onSearch(from, to, suppId)}>🔍 Search</button>
                      ...
                      <tr style={{ background: i % 2 === 0 ? "#fff" : "#fafbff", borderBottom: "1px solid #eaecf4" }}>
                        <td style={{ padding: "5px 6px", textAlign: "center", width: 28 }}>
                          <button onClick={() => toggleExpand(rowId)}
                            style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, color: "#4f8cff", padding: 0,
                              transform: isExpanded ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.15s", display: "inline-block" }}>▶</button>
                        </td>
                        <td style={{ padding: "5px 10px", fontWeight: 700 }}>
  {r.OrderNo || r.PurchaseNo || r.BillNo || "—"}
</td>

<td style={{ padding: "5px 10px" }}>
  {r.Date || r.PurchaseDate || r.BillDate || ""}
</td>

<td style={{ padding: "5px 10px" }}>
  {r.Type || r.PurchaseType || ""}
</td>
<td style={{ padding: "5px 10px" }}>
  {r.SupName || r.SupplierName || r.AccountName || ""}
</td>
                        ...
                        <td style={{ padding: "5px 10px", textAlign: "center" }}>
                          ...
                          <button onClick={() => onDelete(r.Id, r.OrderNo || r.PurchaseNo || r.BillNo)} style={{ padding: "3px 10px", fontSize: 11, borderRadius: 4, border: "1px solid #fecaca", background: "#fee2e2", color: "#dc2626", fontWeight: 600, cursor: "pointer" }}>🗑 Del</button>
                        </td>
                      </tr>
                      {isExpanded && (
                        <tr>
                            ...
                            <div style={{ margin: 10 }}>
                              {rowDetails.length === 0 ? (
                                ...
                                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                                  <thead>
                                    <tr>
                                      {[{ label: "Code", align: "left" }, { label: "Description", align: "left" },
                                        { label: "MRP", align: "right" }, { label: "Pur.Rate", align: "right" },
                                        { label: "Qty", align: "right" }, { label: "GST(%)", align: "right" },
                                        { label: "GST Amt", align: "right" }, { label: "Disc(%)", align: "right" },
                                        { label: "Amount", align: "right" }].map(h => (
    ...
    const handleSave = async () => {
      ...
      try {
        const res = await fetch(VisibleColumnsUrl, {
          method: "POST", headers: { "Content-Type": "application/json; charset=utf-8", ...CC.authHeaders?.() },
  ...
  const RIGHT_KEYS = new Set(["Amount","PurchaseRate","ItemQty","MRP","TaxPercent","DiscountPercent","TaxAmt","DiscountAmt","cdpercent","cdAmount","Pcs","Meter","Noms","LandingCost","ProductTotal","CTAmount","STAmount","Salerate"]);
  ...
  export default function PurchaseOrder() {
    ...
    // ── Settings from localStorage (mirrors PurchaseOrder.js top vars) ──────────
    const [settings] = useState(() => {
      try {
        ...
        return {
          ...
          // Behaviour flags (from PurchaseOrder.js top)
    ...
    const [supplierDetails, setSupplierDetails] = useState({
      address: "", city: "", contactno: "", balance: "0.00",
    });
    ...
    // ── Selected row lookup (mirrors jQuery gridpurchase 'cellselect' →
    //    lblstock.text(getcellvalue(rowindex, grdStockQty)) ) ────────────────────
    const selectedRow = useMemo(
      () => rows.find(r => r._rid === selRid) || null,
      [rows, selRid]
    );
    // Current Stock: stays "0.00" until a product row is actually selected,
    // exactly like lblstock defaulting to "0.00" when ProductRefId is empty.
    const currentStockDisplay = (selectedRow && selectedRow.ProductRefId)
      ...
      : "0.00";
    // Current Rate (Rs): the Pur.Rate of the selected row, same source the
    // grid uses for grdPurchaseRate.
    ...
    // ── Permission check (mirrors PurchaseOrder.js top) ──────────────────────
    useEffect(() => {
    ...
    // ── Supplier Details Load (mirrors SupplierDetailsLoad) ──────────────────
    // const loadSupplierDetails = useCallback((sid) => {
    //   if (!sid) { setSupplierDetails({ address: "", city: "", contactno: "", balance: "0.00" }); return; }
    ...
    const loadSupplierDetails = useCallback((sid) => {
        ...
        if (found) {
          const addr = found.Address2
            ? `${found.Address1},${found.Address2}`
            : (found.Address1 || "");
    ...
    // ── Purchase No mode radio (mirrors rdbMPNo/rdbAPNo) ─────────────────────
    const handlePurchaseNoMode = useCallback((mode) => {
      ...
      if (mode === "MP") {
        ...
        setManualPurchaseNo("");
        setAutoPurchaseNo(purchaseNo);
      }
    }, [purchaseNo]);
  
    // ── PF % / PF Amt logic (mirrors txtpf/txtpfamt keydown) ─────────────────
    const handlePfPerEnter = useCallback(() => {
      const pf = vn(pfPer);
      const net = totals.NetAmt;
      const per = net * pf / 100;
      setPfAmt(per.toFixed(2));
      recalcAllTotals(rowsRef.current, vn(otherPlus), vn(otherMinus), per, igstBill);
    }, [pfPer, totals.NetAmt, otherPlus, otherMinus, igstBill]);
  
    const handlePfAmtEnter = useCallback(() => {
      const pfAmtVal = vn(pfAmt);
      const net = totals.NetAmt;
      const per = net !== 0 ? (pfAmtVal / net) * 100 : 0;
      setPfPer(per.toFixed(2));
      recalcAllTotals(rowsRef.current, vn(otherPlus), vn(otherMinus), pfAmtVal, igstBill);
    }, [pfAmt, totals.NetAmt, otherPlus, otherMinus, igstBill]);
  
    // ── Recalc totals (mirrors methods.Calculation mode=2) ───────────────────
    const recalcAllTotals = useCallback((rowsArr, oPlus, oMinus, pfAmtVal, igst) => {
      ...
      // EcoTech: TSplcess = (otherPlus + pfAmt) * 18/100
      const splcess = settings.EcoTech ? (oPlus + vn(pfAmtVal)) * (18 / 100) : 0;
      const transAmt = settings.EcoTech ? vn(pfAmtVal) : 0;
  
      const netAmt = f2(
        productTotal + gstTotal + splcess + transAmt + oPlus
        - cdTotal - discTotal - oMinus
      );
    ...
    // Recalc on row/state changes
    useEffect(() => {
      recalcAllTotals(rows, vn(otherPlus), vn(otherMinus), vn(pfAmt), igstBill);
      // eslint-disable-next-line
    }, [rows, otherPlus, otherMinus, pfAmt, igstBill]);
  
    // ── Fill item into row (mirrors FillItems) ────────────────────────────────
    const fillItemIntoRow = useCallback((rid, item) => {
      setRows(prev => {
        const newRows = prev.map(r => {
          ...
          // Apply validation: PurchaseRate vs MRP (SaleLandingCostCompare)
          if (settings.SaleLandingCostCompare) {
            if (vn(updated.MRP) < vn(updated.PurchaseRate)) {
        ...
        // Append empty row if this is the last one
        const targetIdx = newRows.findIndex(r => r._rid === rid);
        if (targetIdx === newRows.length - 1 && newRows[targetIdx].ProductRefId) {
          const newRow = mkRow();
    ...
    // ── Fetch product by code (mirrors FillItemsCode/FillItems1) ─────────────
    ...
    const handleCellChange = useCallback((rid, colKey, value) => {
      setRows(prev => prev.map(r => {
        ...
        // ItemQty: validate Noms
        if (colKey === "ItemQty") {
          if ((vn(value) + vn(r.Noms)) === 0) {
            updated.ItemQty = "1";
          }
          const dec = r.UOMDecimal;
          updated.ItemQty = dec === 0 ? Math.round(vn(updated.ItemQty)).toFixed(0) : dec === 2 ? f2(vn(updated.ItemQty)).toFixed(2) : f3(vn(updated.ItemQty)).toFixed(3);
        }
  
        // Validate PurchaseRate vs MRP
        if (colKey === "PurchaseRate" && settings.SaleLandingCostCompare) {
          if (vn(value) > vn(r.MRP)) {
            toast("⚠️ Purchase Rate is Greater than MRP!", true);
          }
        }
  
        if (["ItemQty","PurchaseRate","TaxPercent","DiscountPercent","DiscountAmt","cdpercent","cdAmount","MRP","Noms","Pcs","Meter"].includes(colKey)) {
          return calcPurchaseRow(updated, igstBill);
        }
        return updated;
      }));
    ...
    // ── Cell keydown (mirrors gridpurchase keydown Enter navigation) ──────────
    const handleCellKeyDown = useCallback((e, rid, colKey) => {
      ...
      if (e.key === "Enter") {
        ...
        // PurchaseRate validation (mirrors grdPurchaseRate Enter)
        if (colKey === "PurchaseRate") {
          const row = rowsRef.current.find(r => r._rid === rid);
          if (vn(row?.PurchaseRate) === 0) { toast("Enter the PurchaseRate !!!", true); return; }
          if (settings.SaleLandingCostCompare) {
            if (vn(row?.MRP) < vn(row?.PurchaseRate)) { toast("Purchase Rate is Greater than MRP Cost !!!", true); return; }
          }
        }
  
        // ItemQty validation (mirrors grdItemQty Enter)
        if (colKey === "ItemQty") {
          const row = rowsRef.current.find(r => r._rid === rid);
          if ((vn(row?.ItemQty) + vn(row?.Noms)) === 0) {
            setRows(prev => prev.map(r => r._rid === rid ? { ...r, ItemQty: "1" } : r));
          }
          const dec = rowsRef.current.find(r => r._rid === rid)?.UOMDecimal || 0;
          setRows(prev => prev.map(r => {
            if (r._rid !== rid) return r;
            const v = vn(r.ItemQty);
            const fmt = dec === 0 ? Math.round(v).toFixed(0) : dec === 2 ? v.toFixed(2) : v.toFixed(3);
            return { ...r, ItemQty: fmt };
          }));
        }
  
        // Pcs/Meter → compute qty then move on
        if (colKey === "Pcs" || colKey === "Meter") {
          const row = rowsRef.current.find(r => r._rid === rid);
          if (row) {
            const pcs   = colKey === "Pcs"   ? vn(row.Pcs)   : vn(row.Pcs);
            const meter = colKey === "Meter" ? vn(row.Meter) : vn(row.Meter) || 1;
            const totalqty = pcs * meter;
            const dec = row.UOMDecimal;
            const fmt = dec === 0 ? Math.round(totalqty).toFixed(0) : dec === 2 ? totalqty.toFixed(2) : totalqty.toFixed(3);
    ...
    // ── Save (mirrors methods.PurchaseSave) ──────────────────────────────────
    ...
    const doSave = useCallback(async () => {
        ...
        const purchasemaster = [{
          ...
          PurchaseDetails: purchasedetails.length > 0 ? purchasedetails : [],
        }];
        ...
        const res = await CC.insertapi(PurchaseOrderInsertUrl, purchasemaster, headers);
        setLoading(false);
        if (redirectIfDualLogin(res)) return;
      ...
      }, [perm, editId, gridemptycheck, suppId, suppliers, settings, purchaseNoMode, manualPurchaseNo, autoPurchaseNo, purchaseNo, purchaseDate, purchaseType, igstBill, creditdays, otherPlus, otherMinus, pfPer, pfAmt, totals, remarks, d1, d2, confirm, redirectIfDualLogin, toast]);
      // ── Delete (mirrors F9 in PurchaseOrder.js) ──────────────────────────────
    ...
    const openF5 = useCallback(async (
        from = purchaseDate,
        to = purchaseDate,
        sid = "0"
      ) => {
        ...
        setLoading(true);
        setLdMsg("Loading orders...");
      
        const res = await CC.api(
          PurchaseOrderSelectUrl,
          null,
          {},
          {
        ...
        const master = Array.isArray(res?.Data1) ? res.Data1 : [];
    ...
    // ── Edit (mirrors methods.PurchaseEdit) ──────────────────────────────────
    ...
    const data = Array.isArray(res.Data1)
  ? res.Data1[0]
  : null;
      if (!data) { toast("❌ No data found", true); return; }
  
      const master  = Array.isArray(data) ? data[0] : data;
      const details = master.PurchaseDetails || master.purchasedetails || [];
  
      setEditId(master.Id || id);
  
      if (settings.EcoTech) {
        setPurchaseNo((master.PurchaseNo || "") + settings.BillPrefix);
      } else {
        setPurchaseNo(ns(master.PurchaseNo));
      }
      ...
      setOtherPlus(ns(master.OthersplusAmt || "0.00"));
      ...
      // purchaseeditloaddetails formatting
      const loadedRows = details.map(obj => {
        const formatted = {
          ...
          CTAmount:        f2(vn(obj.CTAmount)),
          STAmount:        f2(vn(obj.STAmount)),
          CTPer:           ns(f2(vn(obj.CTPer))),
          STPer:           ns(f2(vn(obj.STPer))),
          Meter:           ns(f2(vn(obj.Meter))),
          Salerate:        ns(f2(vn(obj.Salerate || obj.SaleRate))),
          ItemQty:         (() => {
    ...
    const clearForm = useCallback(async () => {
      ...
      setPurchaseNoMode("AP");
      setManualPurchaseNo(""); setAutoPurchaseNo("");
      setRows([mkRow()]);
      setSelRid(null);
        ...
        <div className="sb-body">
          ...
          <div style={{ display: "flex", gap: 10, padding: "8px 10px", background: "#f5f8fd", borderBottom: "1px solid #d0ddf5", alignItems: "stretch", flexWrap: "wrap" }}>
            ...
            <div style={{ ...panelStyle, minWidth: 240, flexShrink: 0 }}>
              ...
              {/* Purchase No (Auto/Manual) */}
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <label style={{ ...fieldLabel, minWidth: 78 }}>PurchaseNo</label>
                <input style={{ ...fieldInput, flex: 1 }} value={purchaseNoMode === "MP" ? manualPurchaseNo : autoPurchaseNo}
            ...
            <div style={{ ...panelStyle, flex: 1, minWidth: 260 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                ...
                {/* <select ref={suppRef} style={{ ...fieldInput, flex: 1 }} value={suppId}
                  onChange={e => handleSupplierChange(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && document.getElementById("po-poreq")?.focus()}>
    ...
    onEnterKey={() =>
      document.getElementById("po-poreq")?.focus()
    }
  ...
  />
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <label style={{ ...fieldLabel, minWidth: 72 }}>Current Balance</label>
                <span style={{ fontWeight: 700, color: "#1a2e4a" }}>₹{supplierDetails.balance}</span>
              </div>
            </div>
  
            {/* ── RIGHT PANEL: EcoTech / Remarks / Delivery ── */}
  ...
  <span style={{ color: "#4a5568", fontWeight: 600 }}>
    Current Stock:
    <span style={{ color: "#1a2e4a", fontWeight: 700, marginLeft: 4 }}>
      {currentStockDisplay}
    </span>
  </span>
    ...
    <span style={{ color: "#1a2e4a", fontWeight: 700, marginLeft: 4 }}>
      ₹{currentRateDisplay}
          ...
          {/* ══════════════════════════════════════════════════════════════════
              GRID
          ══════════════════════════════════════════════════════════════════ */}
          {/* <div style={{ display: "flex", gap: 16, alignItems: "center", padding: "3px 8px", background: "#edf2fb", borderBottom: "1px solid #d0ddf5", fontSize: 11 }}>
            <span style={{ color: "#4a5568", fontWeight: 600 }}>
              Current Stock: <span style={{ color: "#1a2e4a", fontWeight: 700 }}>{currentStockDisplay}</span>
            </span>
            <span style={{ color: "#4a5568", fontWeight: 600 }}>
              Rs: <span style={{ color: "#1a2e4a", fontWeight: 700 }}>₹{currentRateDisplay}</span>
            </span>
          </div> */}
                  ...
                  {rows.map((row, idx) => (
                    <tr key={row._rid}
                      ...
                      {visCols.map(vc => {
                        ...
                        const isNum = def?.type === "float" || def?.type === "int";

                        return (
                          <td key={vc.key} style={{ textAlign: isRight ? "right" : undefined }}>
                            {isRO ? (
                              <span className="sb-cell-calc" style={{ display: "block", padding: "0 4px" }}>
                                {typeof val === "number" ? val.toFixed(2) : ns(val)}
                              </span>
                            ) : (
                              <input
                                ...
                                style={{
                                  ...
                                  background: row.ProductRefId && vc.key === "ProductCode" ? "#f0f7ff" : undefined,
                                }}
                                onChange={e => handleCellChange(row._rid, vc.key, e.target.value)}
                                onKeyDown={e => handleCellKeyDown(e, row._rid, vc.key)}
                                onFocus={e => { e.target.select?.(); setSelRid(row._rid); }}
                              />
                            )}
                          </td>
                        );
          ...
          ══════════════════════════════════════════════════════════════════ */}
          <div style={{ display: "flex", borderTop: "1px solid #d0ddf5", background: "#f8faff", minHeight: 96 }}>
  
            {/* GST Split */}
            <div style={{ width: 320, borderRight: "1px solid #d0ddf5", display: "flex", flexDirection: "column", flexShrink: 0 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: "#4a5568", padding: "3px 8px", background: "#edf2fb", borderBottom: "1px solid #d0ddf5" }}>
                GST Split
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", fontSize: 10, fontWeight: 700, color: "#4a5568", padding: "2px 6px", borderBottom: "1px solid #e2e8f0", background: "#f5f8fd" }}>
                <span>GST%</span>
              ...
              <div style={{ overflowY: "auto", flex: 1 }}>
                {gstSplit.map((g, i) => (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", fontSize: 11, padding: "2px 6px", borderBottom: "1px solid #f0f4fc", background: i % 2 === 0 ? "#fff" : "#f9fbff" }}>
                    <span>{f2(g.TaxPercent).toFixed(2)}</span>
            ...
            <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", fontSize: 12 }}>
                  ...
                  <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "4px 10px", borderBottom: "1px solid #edf0f7", gap: 6 }}>
                    ...
                    {item.readOnly
                      ...
                      : <input id={item.id} type={item.type || "text"} step="0.01" value={item.value}
                          ...
                          onKeyDown={e => { if (e.key === "Enter") recalcAllTotals(rowsRef.current, vn(otherPlus), vn(otherMinus), vn(pfAmt), igstBill); }}
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                {[
                  { label: "Total Qty", value: totals.TotalQty.toFixed(3), readOnly: true },
