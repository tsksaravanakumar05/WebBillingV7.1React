...
// Inject keyframe once (identical to BrandMaster)
...
export default function SizeMaster() {
  ...
  // ── Session / permissions (same pattern as BrandMaster, PageName = "Item Master") ──
  const [sess] = useState(() => {
    try {
      ...
      return {
        ...
        IdComList,
        MirrorTable,
        // jQuery uses "Item Master" as the PageName for SizeMaster permissions
  ...
  // SizeMaster-specific: field is SizeName (not BrandName)
  const makeNewRow = (prefill = "") => ({
    Id: null, SizeName: prefill, Active: true, EditMode: 1, _uid: uid(),
  });

  // ── loadData (same pattern as BrandMaster, SizeMaster endpoints) ───────────
  const loadData = useCallback(async () => {
    const prefill = sessionStorage.getItem("masterPrefill") || "";
    setLoading(true);

    // jQuery: $.ajax({ url: "/SizeMaster/SelectSizeMaster", data: '{"Comid":' + Comid + '}' })
    const res = await api(
      "/SizeMaster/SelectSizeMaster",
      null,
      {},
      { Comid: sess.Comid }
    );
    ...
    if (res._http404) { toast("❌ 404 — /SizeMaster/SelectSizeMaster not found", true); }
  ...
  // ── addRow (identical pattern to BrandMaster) ──────────────────────────────
  ...
  const deleteRow = useCallback(async (idx) => {
    ...
    if (isSaved) {
      ...
      // jQuery: unsaved row — just remove from grid
      setGrid(prev => {
        const next = prev.filter((_, i) => i !== idx);
        const sel  = Math.max(0, next.length - 1);
        setSelIdx(sel); focusRow(sel);
  ...
  // ── gridemptycheck (mirrors jQuery gridemptycheck exactly) ─────────────────
  // jQuery: removes last blank row, then validates all EditMode==1 rows have SizeName
  const gridemptycheck = useCallback((g) => {
    let cleaned = [...g];
    // jQuery: if last row SizeName is empty and rowcount > 1, deleterow
    ...
    for (let i = 0; i < cleaned.length; i++) {
      if (cleaned[i].EditMode === 1 && !String(cleaned[i].SizeName || "").trim()) {
        // jQuery: MsgBox("Enter All Size Name in the Grid !!!.")
  ...
  // ── hasDuplicate (mirrors jQuery CheckDuplicate on grdSizeName) ────────────
  const hasDuplicate = useCallback((g) => {
    const names = g.filter(r => String(r.SizeName || "").trim())
                   .map(r => String(r.SizeName).trim().toLowerCase());
    return new Set(names).size !== names.length;
  }, []);

  // ── handleSave (same flow as BrandMaster, all jQuery F1 logic preserved) ───
  const handleSave = useCallback(async () => {
    ...
    // jQuery: permission checks — exact same conditions
    ...
    // jQuery: if flag==0, call Addrowfunc() then return
    if (flag === 0) { addRow(); return; }

    // jQuery: CheckDuplicate(gridSize, grdSizeName, "Brand Name") — note: jQuery uses "Brand Name" label even for Size
    if (hasDuplicate(cleaned)) { toast("❌ Duplicate Size Name found !!!", true); return; }

    // jQuery: var str = 'Do you Want to Save the Size Details?'
    ...
    // jQuery: url: "/SizeMaster/InsertSizeMaster"
    ...
    if (res.ok || res.IsSuccess) {
      ...
      // jQuery: popup/dialog close logic (POPStatus handling)
  ...
  // ── handleEsc (jQuery Esc key — quit page) ─────────────────────────────────
  const handleEsc = useCallback(async () => {
    // jQuery: MsgBoxYesNo("Do You Want To Quit Page?")
    const ok = await confirm("Do You Want To Quit Page?");
    if (!ok) return;
    sessionStorage.removeItem("masterReturnField");
    sessionStorage.removeItem("masterPrefill");
    // jQuery: window.location.href = "/Home" (or close dialog if POPStatus == "ON")
    navigate(-1);
  }, [confirm, navigate]);
  ...
  // Mirrors jQuery gridSize.bind('keydown') logic for grdSizeName column
  const onCellKeyDown = useCallback((e, idx) => {
    if (e.key === "Enter") {
      e.preventDefault();
      // jQuery: if value == null || value == "" → MsgBox("Enter Size Name!!!.")
    ...
    // jQuery: key === 46 (Delete) — Ctrl+Delete for saved rows, plain Delete for unsaved
      ...
      <div className="mp-hdr">
        <div className="mp-hdr-left">
          ...
          <div>
            ...
            <div className="mp-sub">Co: {sess.Comid} — Manage size records</div>
          </div>
        </div>
            ...
            <tbody>
              {grid.map((row, idx) => (
                ...
                >
                  ...
                  {/* SizeName column — mirrors jQuery grdSizeName */}
                  <td>
                    <input
                      ...
                      value={row.SizeName || ""}
                      maxLength={200}
                      onChange={e => applyUppercase(e, (val) => updateCell(idx, "SizeName", val))}
