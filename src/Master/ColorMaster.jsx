...
//
//  Structure mirrors DepartmentMaster exactly:
...
//   • Keyboard: F1 = Save | Esc = Back | Enter = next/add row | Ctrl+Delete = delete row
//
...
//  control flow are 100% unchanged from the original ColorMaster.jsx.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── Column config ────────────────────────────────────────────────────────────
//  Color has two visible columns: Name + Active
const ALL_COLUMNS = [
  { field: "ColorName", label: "Color Name", width: 100 },
...
const redirectIfDualLogin = useCallback((res) => {
  ...
  const rowValidator = useCallback((row) =>
    String(row.ColorName || "").trim().length > 0
  , []);
...
if (redirectIfDualLogin(res)) return;
  ...
  const enableEdit = useCallback((idx) => {
    ...
    selectRow(idx);
    focusRow(idx, 0); // col 0 = ColorName
  }, [focusRow, selectRow]);
...
if (redirectIfDualLogin(res)) return;
  ...
  // ── handleEsc (jQuery Esc — no confirm for Department/Color pattern) ────────
  ...
  const onCellKeyDown = useCallback((e, idx) => {
    if (e.key === "Enter") {
      ...
      if (!String(grid[idx]?.ColorName || "").trim()) {
        toast("❌ Enter Color Name !!!", true); return;
      }
      ...
      <div className="bm-layout">
        <div className="bm-card">
          <div className="bm-card-header">
            <div className="bm-card-header-title">Color Master</div>
            <button type="button" className="bm-close-x" aria-label="Close" onClick={handleEsc}>✕</button>
          </div>

          <div className="bm-card-body">
            <div className="bm-report-title">Color Master</div>
            ...
            <div className="bm-grid-wrap">
              ...
              {grid.length === 0 && !loading && (
                <div className="bm-empty">No records. Press ➕ Add Row to add a color.</div>
              )}
            </div>
