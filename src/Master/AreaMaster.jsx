...
export default function AreaMaster() {
  ...
  const [sess] = useState(() => CC.buildSession("Area"));
  ...
  useEffect(() => {
    ...
    const menudata = menulist.filter(obj => obj.PageName === "Customer"); // "Customer" matches original JS
  ...
  // ── Column config ────────────────────────────────────────────────────────────
  const ALL_COLUMNS = [
    { field: "AreaName", label: "Area Name", width: 300, hidden: false },
  ...
  useEffect(() => {
    const loadColSettings = async () => {
      try {
        const url = `/Content/Appdata/Visible/${sess.MComid}/Area.json?t=${Date.now()}`;
  ...
  const rowValidator = useCallback((row) => {
    return String(row.AreaName || "").trim().length > 0;
  }, []);
  ...
  const loadData = useCallback(async () => {
    ...
    if (res._http404) { toast(`❌ 404 — ${CC.AreaSelect} not found`, true); }
  ...
  const deleteRow = useCallback(async (idx) => {
    ...
    if (isSaved) {
      const ok = await confirm(`Do you want to delete "${row.AreaName}"?`);
      if (!ok) return;
  ...
  const gridemptycheck = useCallback((g) => {
    ...
    if (cleaned.length > 1 && !String(cleaned[cleaned.length - 1].AreaName || "").trim())
      cleaned = cleaned.slice(0, -1);

    for (let i = 0; i < cleaned.length; i++) {
      if (cleaned[i].EditMode === 1 && !String(cleaned[i].AreaName || "").trim()) {
        toast("❌ Enter All Area Name in the Grid !!!", true);
        setSelIdx(i);
        focusRow(i);
  ...
  const hasDuplicate = useCallback((g) => {
    const names = g
      .filter(r => String(r.AreaName || "").trim())
      .map(r => String(r.AreaName).trim().toLowerCase());
  ...
  const handleSave = useCallback(async () => {
    ...
    if (hasDuplicate(cleaned)) { toast("❌ Duplicate Area Name found !!!", true); return; }

    // ── Confirm message ────────────────────────────────────────────────────────
    const hasNew      = dirty.some(r => r.Id == null || r.Id === 0);
    const hasExisting = dirty.some(r => r.Id != null && r.Id !== 0);
    let confirmMsg    = "Do you want to save the Area details?";
    if (hasExisting && !hasNew) confirmMsg = "Do you want to update the Area details?";
    if (hasExisting &&  hasNew) confirmMsg = "Do you want to save & update the Area details?";
    ...
    if (res.IsSuccess || res.ok) {
      ...
      if (retField) {
        ...
        sessionStorage.setItem("masterReturnName",  dirty[0]?.AreaName || "");
  ...
  const onCellKeyDown = useCallback((e, idx) => {
    if (e.key === "Enter") {
      e.preventDefault();
      if (!String(grid[idx]?.AreaName || "").trim()) { toast("❌ Enter Area Name !!!", true); return; }
      if (hasDuplicate(grid))                        { toast("❌ Duplicate Area Name !!!", true); return; }
    ...
    if (e.key === "Delete" && !e.ctrlKey && !String(grid[idx]?.AreaName || "").trim()) {
      e.preventDefault();
      deleteRow(idx);
    }
      ...
      <div className="mp-body">
        ...
        <div className="mp-grid-wrap">
          ...
          {grid.length === 0 && !loading && (
            <div className="mp-empty">No records. Press ➕ to add an area.</div>
          )}
        </div>
