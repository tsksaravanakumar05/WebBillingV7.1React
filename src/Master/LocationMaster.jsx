...
//   • CC.LocationSelect / LocationInsert / LocationDelete endpoint constants
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── Column config ────────────────────────────────────────────────────────────
//  Location has two visible columns: Name + Active
const ALL_COLUMNS = [
  { field: "LocationName", label: "Location Name", width: 300 },
...
export default function LocationMaster() {
  ...
  useEffect(() => {
    ...
    const menudata = menulist.filter(obj => obj.PageName === "Location");
  ...
  // ── selectRow — exits edit mode on non-dirty saved rows (mirrors Dept) ─────
  ...
  const rowValidator = useCallback((row) =>
    String(row.LocationName || "").trim().length > 0
  , []);
  ...
  const enableEdit = useCallback((idx) => {
    ...
    selectRow(idx);
    focusRow(idx, 0); // col 0 = LocationName
  }, [focusRow, selectRow]);
