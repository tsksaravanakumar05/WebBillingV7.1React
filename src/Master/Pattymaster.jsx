// ─────────────────────────────────────────────────────────────────────────────
//  PattyMaster.jsx
//  Ported from frmpattymaster.cs (WinForms) — same grid/edit/save/delete flow
//  as CategoryMaster.jsx. Uses shared helpers from Common.jsx via wildcard
//  import (CC.*). Any new export added to Common is auto-available here as CC.xxx
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── Toggle component (same as CategoryMaster / CashierMaster) ──────────────
...
export default function PattyMaster() {
  ...
  const [sess] = useState(() => {
    try {
      ...
      return {
        ...
        CommonCompany: isCC,
        menudata:     (CC.getLocal("menulist") || []).filter(o => o.PageName === "Patty"),
      };
    } catch {
  ...
  // ─── Column config (matches grdCategory columns in frmpattymaster.cs) ──────
  const ALL_COLUMNS = [
    { field: "PattyName",  label: "Patty Name", width: 260, hidden: false },
    { field: "Percentage", label: "Percentage",  width: 110, hidden: false },
    { field: "Amount",     label: "Amount",      width: 110, hidden: false },
  ...
  const rowValidator = useCallback((row) => {
    return String(row.PattyName || "").trim().length > 0;
  }, []);
  ...
  // ── Permission guard — same logic as CategoryMaster ─────────────────────────
  useEffect(() => {
    ...
    // const menudata = menulist.filter(obj => obj.PageName === "Patty");
  ...
  useEffect(() => {
    const loadColSettings = async () => {
      try {
        const url = `/Content/Appdata/Visible/${sess.MComid}/Patty.json?t=${Date.now()}`;
  ...
  // ── loadData ── (mirrors FormRefresh() → PattySelect?Comid=...) ─────────────
  const loadData = useCallback(async () => {
    ...
    if (res._http404) { toast(`❌ 404 — ${CC.PattySelect} not found`, true); }
  ...
  // ── deleteRow ── (mirrors PattyDelete?Id=... — no Comid param in source) ────
  const deleteRow = useCallback(async (idx) => {
    ...
    if (isSaved) {
      const ok = await confirm(`Wish to Delete the Record "${row.PattyName}" ?`);
      if (!ok) return;
      ...
      if (res.ok || res.IsSuccess) {
        toast("✅ " + (res.message || "Delete Successfully !!!"));
        setGrid(prev => {
  ...
  }, [grid, perm, focusRow, toast, confirm]);

  // ── gridemptycheck ── (mirrors GridEmptyCheck()) ────────────────────────────
  const gridemptycheck = useCallback((g) => {
    let cleaned = [...g];
    if (cleaned.length > 1 && !String(cleaned[cleaned.length - 1].PattyName || "").trim())
      cleaned = cleaned.slice(0, -1);

    for (let i = 0; i < cleaned.length; i++) {
      if (cleaned[i].EditMode === 1 && !String(cleaned[i].PattyName || "").trim()) {
        toast("❌ Enter All Patty Name in the Grid !!!", true);
        setSelIdx(i);
        focusRow(i);
  ...
  // ── hasDuplicate ── (mirrors CheckDuplicate on PattyName) ───────────────────
  const hasDuplicate = useCallback((g) => {
    const names = g
      .filter(r => String(r.PattyName || "").trim())
      .map(r => String(r.PattyName).trim().toLowerCase());
    return new Set(names).size !== names.length;
  }, []);

  // ── handleSave ── (mirrors SaveCategory()) ──────────────────────────────────
  const handleSave = useCallback(async () => {
    ...
    // ── Permission checks (mirrors CategoryMaster / CashierMaster exactly) ──
    if (perm.Add === 0 && perm.Edit === 0) {
      ...
      if (!dirty.length) {
        const any = cleaned.filter(r => r.EditMode === 1);
        toast(any.length ? "❌ Page Add Permission Denied !!!" : "⚠️ No Update", true);
        flag = 0;
      }
    }

    if (flag === 0) { addRow(); return; }
    if (hasDuplicate(cleaned)) { toast("❌ Duplicate Patty Name found !!!", true); return; }

    const proceed = await confirm("Wish to Save Patty Details ?");
    if (!proceed) { addRow(); return; }

    setLoading(true);

    // ── Build API payload (mirrors PattyModel) ──────────────────────────────
    const payload = cleaned
      .filter(r => r.EditMode === 1)
      .map(r => ({
        Id:         Number(r.Id || 0),
        PattyName:  r.PattyName            || "",
  ...
  const onCellKeyDown = useCallback((e, idx) => {
    if (e.key === "Enter") {
      e.preventDefault();
      if (!String(grid[idx]?.PattyName || "").trim()) { toast("❌ Enter Patty Name !!!", true); return; }
      if (hasDuplicate(grid))                          { toast("❌ Duplicate Patty Name !!!", true); return; }
    ...
    if (e.key === "Delete" && !e.ctrlKey && !String(grid[idx]?.PattyName || "").trim()) {
      e.preventDefault();
      deleteRow(idx);
    }
      ...
      <div className="mp-body">
        ...
        <div className="mp-grid-wrap">
                ...
                {visibleColumns.map(c => (
                  <th
                    ...
                    style={{
                      width: c.width,
                      minWidth: c.width,
                      textAlign: (c.field === "Active" || c.field === "Percentage" || c.field === "Amount") ? "center" : undefined,
                    }}
                  >
                    {c.label}
                  </th>
              ...
              {grid.map((row, idx) => (
                ...
                >
                  ...
                  {visibleColumns.map((col, colIdx) => (
                    <td
                      key={col.field}
                      style={{ textAlign: (col.field === "Active" || col.field === "Percentage" || col.field === "Amount") ? "center" : undefined }}
                    >
                      {/* ── Active Toggle ── */}
                      {col.field === "Active" && (
                        <Toggle
                      ...
                      {/* ── Numeric inputs (Percentage / Amount) ── */}
                      {(col.field === "Percentage" || col.field === "Amount") && (
                        <input
                          ref={el => {
                            if (!inputRefs.current[idx]) inputRefs.current[idx] = [];
                            inputRefs.current[idx][colIdx] = el;
                          ...
                          onChange={e => {
                            ...
                            if (/^\d*\.?\d{0,2}$/.test(v)) updateCell(idx, col.field, v);
                          }}
                          onBlur={() => {
                            if (row.EditMode === 1) {
                              updateCell(idx, col.field, parseFloat(row[col.field] || 0).toFixed(2));
                            }
                          }}
                          onKeyDown={e => { if (row.EditMode === 1) onCellKeyDown(e, idx); }}
                          onFocus={() => selectRow(idx)}
                          style={{
                            textAlign:    "right",
          ...
          {grid.length === 0 && !loading && (
            <div className="mp-empty">No records. Press ➕ to add a patty.</div>
          )}
        </div>
